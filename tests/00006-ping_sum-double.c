...
int main (int argc, char **argv) {
    ...
    double number = 0;
    int i, j;
    int *array = (int *)malloc(ARRAY*sizeof(double));
    if (size > 1) {
        if (rank == 0) {
            ...
            for (i = 1; i < size; i++) {
                MPI_Send(array, ARRAY, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
                printf("Process %d sent array to process %d\n", rank, i);
            }
            for (i = 1; i < size; i++) {
                number = 0;
                MPI_Recv(&number, 1, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                printf("Process 0 got sum %d\n", number);
                number += 0.1;
                assert(fabs(number - 285.1) < 1e-5);
            }
        } else {
            MPI_Recv(array, ARRAY, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
